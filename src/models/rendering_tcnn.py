...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, sigma_only=False, num_colors=3, softplus=False, far=None):
    ...
    if softplus == True:
        alphas = 1-torch.exp(-deltas*torch.nn.functional.softplus(sigmas+noise)) # (N_rays, N_samples_)
    else:
    ...
    if far is not None:
        ...
        weights_appended = torch.cat([weights, 1-weights.sum(dim=1, keepdim=True)], dim=1)
        depth_map = torch.sum(weights_appended*z_vals_appended, -1) #+ (1 - weights_sum) * 100
    else:
...
def render_rays(rays,
    ...
    def inference(model, xyz_, dir_, sigma_only=False):
        ...
        if sigma_only:
            ...
            dir_ = torch.repeat_interleave(dir_, repeats=N_samples_, dim=0).contiguous() # (N_rays*N_samples_, embed_dir_channels)
...
def render_rays_cf(rays,
    ...
    xyz_samples_coarse = rays_o.unsqueeze(1) + rays_d.unsqueeze(1) * z_vals.unsqueeze(2) # (N_rays, N_samples, 3)

    raw = inference(nerf_model_coarse, xyz_samples_coarse, viewdirs, sigma_only=sigma_only)
    
    rgb_coarse, depth_coarse, weights_coarse, opacity_coarse = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, sigma_only=sigma_only, num_colors=num_colors, far=far)
    ...
    if N_importance > 0:
        ...
        z_vals_ = sample_pdf(z_vals_mid, weights_coarse[:, 1:-1], N_importance, det=False).detach()
        ...
        xyz_samples_fine = rays_o.unsqueeze(1) + rays_d.unsqueeze(1) * z_vals.unsqueeze(2) # (N_rays, N_samples+N_importance, 3)

        raw = inference(nerf_model_fine, xyz_samples_fine, viewdirs, sigma_only=sigma_only)

        rgb_fine, depth_fine, weights_fine, opacity_fine = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, sigma_only=sigma_only, num_colors=num_colors, far=far)
