...
class OccGridRaySampler():
    ...
    def get_samples(self, rays, N_samples, perturb):
        ...
        z_steps = torch.linspace(0, 1, N_samples // 2, device=rays.device)        # (N_samples)
        # z_steps = torch.logspace(-4, 0, N_samples, device=rays.device)       # (N_samples)
        z_vals = near * (1-z_steps) + far * z_steps
        ...
        with torch.no_grad():
            ...
            z_vals_mid = 0.5 * (z_vals[: ,:-1] + z_vals[: ,1:]) # (N_rays, N_samples-1) interval mid points
            z_vals_ = sample_pdf(z_vals_mid, point_probs[:, 1:-1], N_samples // 2, det=False).detach()
                      # detach so that grad doesn't propogate to weights_coarse from here

            z_vals, _ = torch.sort(torch.cat([z_vals, z_vals_], -1), -1)  # sorting is important!
