"""
File: examples/fusion_portable/fusion_portable_calibration.py
...
class FusionPortableCalibration:
    ...
    def to_dict(self, im_scale_factor):
        result = {"camera_intrinsic": {}}

        K = torch.from_numpy(self.left_cam_intrinsic["K"]).float()
        K[:2, :] *= im_scale_factor
        result["camera_intrinsic"]["k"] = K

        new_k = torch.from_numpy(self.left_cam_intrinsic["projection_matrix"]).float()[:3,:3]
        new_k[:2, :] *= im_scale_factor
        result["camera_intrinsic"]["new_k"] = new_k

        result["camera_intrinsic"]["distortion"] = \
            torch.from_numpy(self.left_cam_intrinsic["distortion_coeffs"]).float()

        result["camera_intrinsic"]["width"] = int(self.left_cam_intrinsic["width"] // (1/im_scale_factor))
        
        result["camera_intrinsic"]["height"] = int(self.left_cam_intrinsic["height"] // (1/im_scale_factor))

        result["lidar_to_camera"] = self.t_lidar_to_left_cam
